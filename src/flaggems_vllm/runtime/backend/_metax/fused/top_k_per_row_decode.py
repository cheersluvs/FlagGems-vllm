...
the installed FlagTree passes top_k_per_row_tle's self-test (0.964 -> 1.884 of
vLLM, kernel mode). Otherwise exactly the generic non-TLE path.

The non-TLE row split that used to live here (two passes over the generic
kernel on chunked views) has been removed; see git history for it and its
measurements."""
