...
Measured on a C550 against vLLM's own kernels, kernel mode:

    path            decode   prefill
    non-TLE (was)    0.964     0.640
    TLE, this file   1.760     1.194

The whole difference is where the histogram and the write counters live:
...
WHICH BUILDS QUALIFY. The TLE path needs three things the published MetaX
FlagTree wheels do not all have, so this file never assumes them -- it runs a
...
WHAT GETS CHANGED, by rebinding the generic modules' globals (no generic diff):
  ...
  NUM_THREADS_PER_BLOCK_MERGE / _LAUNCH_GEOMETRY
             ...
             limit: 512"), while torch's device properties say more. Shrink
             the tile, not only the warps: 1024 lanes on 8 warps would be 2
             elements per thread.
