...
"""T-Head override: encode OCP E4M3 with integer arithmetic.
...
`tl.float8e4nv` cannot be compiled for this card. PPU-ZW810E reports capability
(8, 0) and its Triton target is `ppu:80`, and the fp8 policy is inherited from the
NVIDIA backend at two levels:

  * FlagTree `third_party/ppu/backend/compiler.py` adds "fp8e4nv" to
    `supported_fp8_dtypes` only when `capability >= 89`;
  * overriding that option to get past the frontend then fails in the PPU
    lowering pass with `LLVM ERROR: Conversion from/to f8e4m3nv is only supported
    on compute capability >= 89` -- an NVIDIA threshold, and an NVIDIA concept,
    enforced inside `ConvertTritonGPUToLLVMPPU`.

So the format is refused by policy rather than shown to be unsupported. This
encoder sidesteps the question: it produces the E4M3 byte with integer ops, which
is what a software conversion does anyway below sm_89 -- torch's own cast on this
card takes such a path. Verified bit-identical to `torch.float8_e4m3fn` over 65536
values, half of them drawn from the subnormal range below 2^-6, which is the part
a naive encoder gets wrong.

Delete this file once the PPU backend either lowers `fp8e4nv` or reports a target
that permits it. Nothing else here differs from the generic implementation.
"""
