...
def profile_one(rows, vocab, top_k):
    ...
    print(f"  ({rows},{vocab}) k={top_k}: device {tot:.4f} ms over "
          f"{sum(e.count for e in evs) / 10:.1f} launches")
    for e in evs[:4]:
        print(f"      {e.self_device_time_total / 10 / 1000:8.4f} ms  "
...
def main():
    ...
    print("=" * 76)
    print(f"  {'vocab':>8} {'rows':>6} {'ms':>9} {'us/prog':>9} {'model':>8} {'diff':>8}")
    for vocab in (1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072):
        ...
        print(f"  {vocab:>8} {rows:>6} {t:>9.4f} {pp:>9.2f} {model:>8.2f} "
              f"{pp - model:>+8.2f}")
    ...
    for vocab in (4096, 32768, 131072):
        ...
        for top_k in (128, 256, 512, 1024):
            ...
            print(f"  {vocab:>8} {top_k:>6} {pp:>9.2f}"
